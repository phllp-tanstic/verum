...
const modulePreloadLinkRE = /<link(?![\w-])[^>]*?\srel=(['"])modulepreload\1[^>]*>/g;
...
function viteLegacyPlugin(options = {}) {
	...
	const facadeToLegacyImportMap = /* @__PURE__ */ new Map();
	...
	return [
		...
		},
		{
			...
			configResolved(_config) {
				...
				if (Array.isArray(output)) rolldownOptions.output = [...output.map(createLegacyOutput), ...genModern ? output : []];
				else rolldownOptions.output = [createLegacyOutput(output), ...genModern ? [output || {}] : []];
			...
			transformIndexHtml(html, { chunk, bundle }) {
				...
				if (chunk.fileName.includes("-legacy")) {
					...
					if (config.build.chunkImportMap) facadeToLegacyImportMap.set(chunk.facadeModuleId, bundle[getImportMapFilename(config)]);
					if (genModern) return;
				}
				if (!genModern) html = html.replace(/<script type="module".*?<\/script>/g, "").replace(modulePreloadLinkRE, "");
				const tags = [];
			...
			generateBundle(_opts, bundle) {
				...
				if (isLegacyBundle(bundle) && genModern) {
					...
					for (const name in bundle) if (bundle[name].type === "asset" && !name.endsWith(".map") && !name.includes("-legacy") && name !== importMapFilename) delete bundle[name];
				}
			}
		}
	];
...
//#endregion
export { cspHashes, viteLegacyPlugin as default, detectPolyfills, viteLegacyPluginCjs as "module.exports", modulePreloadLinkRE };
